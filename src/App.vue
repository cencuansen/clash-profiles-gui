<template>
  <div class="container">
    <div class="header">
      <el-button size="small" type="primary" @click="loadFile">
        <el-icon>
          <i-ep-folder-opened />
        </el-icon>
      </el-button>
          ...
          <el-collapse-item v-for="(g, index) in groups" :key="index" :name="g.name">
              ...
              <div class="group-header">
                ...
                <div>
                  ...
                  <el-button
                    ...
                    size="small"
                    class="group-delete"
                    @click.stop="deleteGroup(g.name, index)"
                  >
                    <el-icon>
                      <i-ep-delete />
                    </el-icon>
            ...
            <el-checkbox-group v-model="g.proxies" @change="updateGroupProxies(g.name, $event)">
              <div class="proxy-row" v-for="(p, pIndex) in paginatedGroupProxies[g.name]" :key="p">
                <el-checkbox :value="p" :disabled="p === 'DIRECT'">{{ p }}</el-checkbox>
              </div>
            </el-checkbox-group>
            <el-pagination
              v-model:current-page="groupCurrentPage[g.name]"
              :page-size="groupPageSize"
              :total="(proxies?.length || 0) + 1"
              layout="prev, pager, next"
              size="small"
              @current-change="handleGroupPageChange(g.name)"
            ></el-pagination>
          </el-collapse-item>
        </el-collapse>
      </div>
          ...
          <el-collapse-item v-for="type in ruleTypes" :key="type" :name="type" :title="type">
            <template #title>
              <div class="rule-header">
                ...
                <el-select
                  v-if="(rules[type]?.length || 0) > 0"
                  v-model="batchGroup[type]"
                  size="small"
                  placeholder="选择分组"
                  class="batch-group"
                  clearable
            ...
            <div class="rule-input">
              <el-input
                ...
                placeholder="输入"
                clearable
                @input="debouncedFilterRules(type)"
                @keyup.enter="addRule(type)"
              ></el-input>
              <el-button class="add-rule" type="primary" size="small" @click="addRule(type)">
                <el-icon>
                  <i-ep-plus />
                </el-icon>
              </el-button>
            ...
            <div class="rule-content">
              <div class="rule-row" v-for="(r, index) in paginatedRules[type]" :key="index">
                ...
                <el-button
                  type="danger"
                  size="small"
                  @click="deleteRule(type, rules[type].indexOf(r.rule))"
                >
                  <el-icon>
                    <i-ep-delete />
                  </el-icon>
            ...
            <el-pagination
              v-model:current-page="currentPage[type]"
              :page-size="pageSize"
              :total="filteredRules[type].length"
              layout="prev, pager, next"
              size="small"
              @current-change="handlePageChange(type)"
            ></el-pagination>
          </el-collapse-item>
        </el-collapse>
      </div>
...
const groupTypes = ['url-test', 'fallback', 'select', 'load-balance']
const proxies = ref([])
const groups = ref([])
...
const filterRules = type => {
  ...
  filteredRules.value[type] = query
    ...
    : rules.value[type].map((rule, index) => ({
        rule,
        group: rules.value[type + '_group'][index] || defaultGroup
      }))
  currentPage.value[type] = 1
...
ipcRenderer.on('profile-loaded', (event, content) => {
  ...
  ruleTypes.forEach(type => {
    ...
    rules.value[type + '_group'] =
      groupsByType[type] || rulesByType[type]?.map(() => defaultGroup) || []
...
const updatePaginatedGroupProxies = groupName => {
  ...
  const groupProxies = groups.value
    .filter(x => x.name === groupName)[0]
    .proxies.filter(x => x !== DIRECT && !allProxies.includes(x))
